...
- Real authentication-state mutation remains disabled until versioned `agy` capability evidence is
  approved in an ADR. ADR 0007 permits an unofficial isolated-home contract only after its black-box
  gates pass; filenames and credential contents alone are never a contract.
