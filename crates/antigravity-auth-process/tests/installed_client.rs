...
use antigravity_auth_process::{OfficialClient, discover_client};
use std::path::Path;
use std::time::Duration;

#[test]
#[ignore = "requires ANTIGRAVITY_AUTH_TEST_AGY pointing to an installed agy executable"]
fn discovers_and_probes_installed_agy_without_authentication() {
    let path = std::env::var_os("ANTIGRAVITY_AUTH_TEST_AGY")
        .expect("ANTIGRAVITY_AUTH_TEST_AGY must be set");
    ...
    if let Ok(expected) = std::env::var("ANTIGRAVITY_AUTH_TEST_AGY_VERSION") {
